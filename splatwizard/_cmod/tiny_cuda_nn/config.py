...
# from diffusers.pipelines.pipeline_utils import LIBRARIES
...
def _stub_dir():
    ...
    assert maybe_nvcc is not None, "NVCC not found"

    cuda_home = Path(maybe_nvcc).parent.parent

    stub_dir = cuda_home / 'lib64' / 'stubs'
    return str(stub_dir.absolute())
...
third_party_dir = module_path.parent.parent / "third_party"

third_party_dir_str = str(third_party_dir.absolute())
...
base_source_files = [
    module_path_str + "/bindings.cpp",
    module_path_str + "/src/cpp_api.cu",
    module_path_str + "/src/common_host.cu",
    module_path_str + "/src/encoding.cu",
    module_path_str + "/src/object.cu",
    third_party_dir_str + "/fmt/src/format.cc",
    third_party_dir_str + "/fmt/src/os.cc",
]


base_source_files += [
    module_path_str + "/src/network.cu",
    module_path_str + "/src/cutlass_mlp.cu",
    module_path_str + "/src/fully_fused_mlp.cu"
]
...
#         source_files = base_source_files + [module_path_str + "/src/fully_fused_mlp.cu"]
#     else:
...
#
#     include_dirs = [
#                 f"{module_path_str}/include",
#                 third_party_dir_str,
#                 f"{third_party_dir_str}/cutlass/include",
#                 f"{third_party_dir_str}/cutlass/tools/util/include",
#                 f"{third_party_dir_str}/fmt/include",
#     ]
#     return nvcc_flags, cflags, include_dirs, source_files


NAME = '_tiny_cuda_nn'
definitions = base_definitions + [f"-DTCNN_MIN_GPU_ARCH={compute_capability}"]

NVCC_FLAGS = base_nvcc_flags + definitions
CXX_FLAGS = base_cflags + definitions
EXTRA_INCLUDE_PATHS = [
    f"{module_path_str}/include",
    third_party_dir_str,
    f"{third_party_dir_str}/cutlass/include",
