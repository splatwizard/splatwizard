...
from splatwizard.utils.encodings import use_clamp
...
class EntropyGaussianClamp(nn.Module):
    def __init__(self, Q=1):
        super(EntropyGaussianClamp, self).__init__()
        self.Q = Q
    def forward(self, x, mean, scale, Q=None):
        ...
        likelihood = LowBound.apply(likelihood)
...
class EntropyBernoulli(nn.Module):
    def __init__(self):
        super().__init__()
...
class EntropyFactorized(nn.Module):
    def __init__(self, channel=32, init_scale=10, filters=(3, 3, 3), likelihood_bound=1e-6,
        ...
        super(EntropyFactorized, self).__init__()
...
class LowBound(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x):
