...
from .utils import read_ply_data, write_ply_data, load_vqgaussian, build_ply_data
...
class VecTreeConfig:
    ...
    vq_ratio: float = 0.6
    codebook_size: int =2 ** 13  # 2**13 = 8192
    vq_chunk: int = 80000
    no_IS: bool = False
    vq_way: str = 'half'
    device: str = 'cuda'
...
class VecTreeCodec:
    def __init__(self, opt: VecTreeConfig = VecTreeConfig()):
    ...
    def setup(self, feats):
        ...
        self.model_vq = VectorQuantize(
            dim=self.feats.shape[1],
            codebook_size=self.opt.codebook_size,
            decay=0.8,
            commitment_weight=1.0,
            use_cosine_sim=False,
            threshold_ema_dead_code=0,
        ...
        logger.info(wrap_str("Input_feats_shape: ", self.feats_bak.shape))
        logger.info(wrap_str("VQ_feats_shape: ", self.feats.shape))
        logger.info(wrap_str("SH_degree: ", self.opt.sh_degree))
        logger.info(wrap_str("Quantization_ratio: ", self.opt.vq_ratio))
        logger.info(wrap_str("Add_important_score: ", self.opt.no_IS == False))
        logger.info(wrap_str("Codebook_size: ", self.opt.codebook_size))
        # logger.info("================================================ ")
    ...
    def calc_vector_quantized_feature(self):
        ...
        for i in tqdm(range(0, self.feats.shape[0], CHUNK)):
            feat, indices, commit = self.model_vq(self.feats[i:i + CHUNK, :].unsqueeze(0).to(self.opt.device))
    ...
    def fully_vq_reformat(self):
        ...
        def savez_and_log_size_info(file, *args, **kwargs):
            np.savez_compressed(file,*args,  **kwargs)
            logger.info(wrap_str(file, os.path.getsize(file) / 1024, 'KB'))
        ...
        with TemporaryDirectory() as dirname:
            ...
            savez_and_log_size_info(f'{save_path}/extreme_saving/metadata.npz', metadata=metadata)
            ...
            bin_indices = dec2bin(self.codebook_vq_index,
                                  int(math.log2(self.codebook_size))).bool().cpu().numpy()  # mapping_index
            savez_and_log_size_info(f'{save_path}/extreme_saving/vq_indexs.npz', np.packbits(bin_indices.reshape(-1)))

            # ----- save codebook -----
            codebook = self.model_vq._codebook.embed.cpu().half().numpy().squeeze(0)
            savez_and_log_size_info(f'{save_path}/extreme_saving/codebook.npz', codebook)

            # ----- save keep mask (non_vq_feats_index)-----
            savez_and_log_size_info(f'{save_path}/extreme_saving/non_vq_mask.npz',
                                np.packbits(self.non_vq_mask.reshape(-1).cpu().numpy()))

            # ===================================================== save non_vq_SH =============================================
            non_vq_feats = self.feats_bak[self.non_vq_mask, 6:6 + self.sh_dim]
            wage_non_vq_feats = self.wage_vq(non_vq_feats)
            savez_and_log_size_info(f'{save_path}/extreme_saving/non_vq_feats.npz', wage_non_vq_feats)
            ...
            savez_and_log_size_info(f'{save_path}/extreme_saving/other_attribute.npz', wage_other_attribute)

            xyz = self.feats_bak[:, 0:3]
            savez_and_log_size_info(f'{save_path}/extreme_saving/xyz.npz', xyz)

            # zip everything together to get final size
            os.system(f"zip -r -j {save_path}/extreme_saving.zip {save_path}/extreme_saving")
            size = os.path.getsize(f'{save_path}/extreme_saving.zip')

            with open(f'{save_path}/extreme_saving.zip', 'rb') as f:
                bitstream = f.read()
        size_MB = size / 1024.0 / 1024.0
        logger.info("Size = {:.2f} MB".format(size_MB))

        return all_feat, all_indice, bitstream
    ...
    def quantize(self, importance=None):
        if self.no_IS or importance is None:  # no important score
        ...
        logger.info(wrap_str("IS_percent: ", IS_percent))
        ...
        with torch.no_grad():
            ...
            for i in trange(self.iteration_num):
                ...
                vq_weight = imp[indexes].to(self.opt.device)
                vq_feature = feats_needs_vq[indexes, :].to(self.opt.device)
        ...
        all_feat, all_indices, bitstream = self.fully_vq_reformat()

        return bitstream

    def dequantize(self, bitstream):
        print("\n==================== Load saved data & Dequantize ==================== ")

        with TemporaryDirectory() as dirname:
            with open(f'{dirname}/extreme_saving.zip', 'wb') as f:
                f.write(bitstream)

            os.system(f'unzip {dirname}/extreme_saving.zip -d {dirname}/extreme_saving')
            dequantized_feats = load_vqgaussian(f'{dirname}/extreme_saving', device=self.opt.device)
